...
func readData(rw *bufio.ReadWriter) {
	if IDname == "0" {
		...
		outputFile, outputError := os.OpenFile(CaculatePath()+"2.mp3", os.O_WRONLY|os.O_CREATE, 0666)
		if outputError != nil {
...
func writeData(rw *bufio.ReadWriter) {
	if IDname == "1" {
		file, err := os.Open(CaculatePath() + "1.mp3")
		if err != nil {
			panic(err)
		}
		defer file.Close()
...
func main() {
	...
	IDname = strconv.Itoa(*listenF % 2)

	if *listenF == 0 {
		log.Fatal("Please provide a port to bind on with -l \n start with go run main.go -l 10000 -secio")
	}
